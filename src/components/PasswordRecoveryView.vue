...
<v-row justify="center">
    <v-col width="500">
        <v-dialog v-model="this.$store.state.passwordRecoveryDialog" persistent>
            <v-card>
                <v-card-title class="headline">Account Recovery</v-card-title>
                <v-stepper v-model="step">
                    <v-stepper-header>
                        <v-stepper-step color="green" :complete="step > 1" step="1">CellPhone Number</v-stepper-step>
                        <v-divider></v-divider>
                        <v-stepper-step color="green" :complete="step > 2" step="2">Recovery Code</v-stepper-step>
                        <v-divider></v-divider>
                    </v-stepper-header>
                    <v-stepper-items>
                        <v-stepper-content step="1">
                            <v-form ref="form" v-model="valid" lazy-validation>
                                ...
                                <v-select :items="countries" label="Country" :rules="countryRules" v-model="selectedCountry"></v-select>
                            </v-form>
                            <v-btn v-if="valid" color="green" @click="validateRecoveryDetails">
                                Continue
                            </v-btn>
                            <v-btn text :ripple="{ center: true }" color="black" @click="closeView">Cancel</v-btn>
                        </v-stepper-content>
                        <v-stepper-content step="2">
                            <v-text-field v-model="recoveryCode" label="Recovery Code" required></v-text-field>
                            <v-btn color="green" @click="step = 2">
                                Continue
                            </v-btn>
                            <v-btn text :ripple="{ center: true }" color="black" @click="closeView">Cancel</v-btn>
                        </v-stepper-content>
